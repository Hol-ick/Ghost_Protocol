"""Legacy operator console (preserved). New default UI: ghost_protocol.studio.
...
def _handle_infinite_batch_done(
    ...
    if missing:
        ...
        if int(ss.get("_infinite_refill_round", 0) or 0) < max_refill_rounds:
            observability.append_event(
                ss,
                kind="refill_start",
                title="refill missing drafts",
                detail=f"missing={missing} target={target} round_limit={max_refill_rounds}",
                status="warn",
                cycle=cycle_no,
            )
            _start_infinite_refill_batch(ss, missing)
            return "refilling"
        ss.setdefault("swarm_log", []).append(
            f"[∞] 보충 라운드 상한 도달 — 부족 {missing}개, "
            f"상한 {max_refill_rounds}회. 현재 성공분 기준으로 진행합니다."
        )
        observability.append_event(
            ss,
            kind="cycle_restart",
...
def _ops_styles() -> str:
    return (
        ...
        ".ops-resource-track{height:7px;border-radius:999px;background:#E2E8F0;margin-top:10px;overflow:hidden;}"
        ".ops-resource-fill{height:100%;border-radius:999px;background:linear-gradient(90deg,#2563EB,#7C3AED);}"
        ".ops-resource-note{margin-top:9px;font-size:11px;font-weight:800;color:#475569;line-height:1.45;overflow-wrap:anywhere;}"
        ".ops-resource-note b{color:#4F46E5;}"
        "</style>"
    )
...
def _render_ops_resource_card() -> str:
    ...
    def delta_text(value: object) -> str:
        if value is None:
            return "기준 없음"
        number = float(value)
    ...
    budget_label = f"{calls}/{budget}" if budget else str(calls)
    ratio = max(0.0, min(1.0, float(summary.get("budget_ratio") or 0.0)))
    ratio_label = f"{ratio * 100:.0f}%" if budget else "제한 없음"
    status = str(summary.get("status") or "idle")
    mode = str(summary.get("mode") or "대기")
    judge_mode = str(summary.get("judge_mode") or "auto")
    cache_min = max(0, int(summary.get("trend_cache_ttl_sec") or 0) // 60)
    saver_label = "절약 ON" if summary.get("cost_saver") else "절약 OFF"
    if summary.get("has_usage_tokens"):
        usage_label = "실측 토큰"
        usage_main = compact_int(summary.get("total_tokens"))
        usage_sub = f"입력 {compact_int(summary.get('prompt_tokens'))}"
    else:
        usage_label = "추정 문자량"
        usage_main = compact_int(summary.get("prompt_chars"))
        usage_sub = f"응답 {compact_int(summary.get('response_chars'))}"
    compare_note = "비교 기준 없음 · 같은 모드 Run이 쌓이면 표시"
    if comparison.get("has_baseline"):
        compare_note = (
            f"이전 평균 대비 호출 {delta_text(comparison.get('call_delta_pct'))} · "
            f"Judge {delta_text(comparison.get('judge_delta_pct'))}"
        )
    fill = (
        f'<div class="ops-resource-track"><div class="ops-resource-fill" '
        f'style="width:{ratio * 100:.1f}%"></div></div>'
        if budget
        else ""
    )
    return (
        ...
        + '</div>'
        + fill
        + f'<div class="ops-resource-note"><b>{_html.escape(mode)}</b> · {saver_label} · Judge { _html.escape(judge_mode) } · 캐시 {cache_min}분</div>'
        + f'<div class="ops-resource-note">{_html.escape(compare_note)}</div>'
        + '</div>'
    )
    ...
    if _ir:
        ...
        try:
            ...
            observability.append_exception_event(
                ss,
                kind="database_error",
                title="AI post lookup failed",
                detail=f"gallery={_intel_gid or '-'}",
                exc=exc,
            )
            _ai_nos_db = set()
...
if fire_clicked:
    ...
    if not has_llm:
        ...
        _run_setup = run_config.build_batch_run_setup(
            ...
            rehearsal_cycle_limit=st.session_state.get("rehearsal_cycle_limit"),
            intel_result=st.session_state.get("intel_result") or {},
            ai_comment_watch_limit=st.session_state.get(
                "ai_comment_watch_limit",
                operator_settings.DEFAULT_AI_COMMENT_WATCH_LIMIT,
            ),
        )
        _actual_count = _run_setup.actual_count
        _rehearsal_cycle_limit = _run_setup.rehearsal_cycle_limit
        _rehearsal_anchor_posts = _run_setup.rehearsal_anchor_posts
        _run_mode = _run_setup.run_mode
        _run_id = observability.start_run(
            st.session_state,
            mode=_run_mode,
            gallery_id=_gallery_id,
        ...
        st.session_state["run_prompt_versions"] = [
            {
                ...
                "style_profile": _run_setup.prompt_version["style_profile"],
                "composition_profile": _run_setup.prompt_version["composition_profile"],
            }
        ]
        ...
        observability.append_event(
            ...
            detail=_run_setup.cycle_start_detail,
            status="running",
            cycle=1,
            metrics={"target": _actual_count},
        )

        st.session_state["_batch_gen_config"] = dict(_run_setup.batch_config)

        st.session_state.swarm_log            = list(_run_setup.initial_log_lines)
        st.session_state["rehearsal_runs"] = []
        st.session_state["test_summaries"] = []
...
if _intel_fire:
    ...
    if not has_llm:
        ...
        _read_run_id = observability.start_run(
            st.session_state,
            mode="read",
            gallery_id=_igid,
