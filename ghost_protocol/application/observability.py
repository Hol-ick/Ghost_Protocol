...
def append_exception_event(
    ...
    *,
    ...
    """Record an exception in the operator timeline without raising again."""

    exc_name = exc.__class__.__name__
    exc_text = compact_text(str(exc), 180)
    suffix = f"{exc_name}: {exc_text}" if exc_text else exc_name
    append_event(
        state,
        kind=kind,
        title=title,
        detail=f"{compact_text(detail, 70)} — {suffix}" if detail else suffix,
        status="error",
        cycle=cycle,
        metrics={"exception": exc_name},
    )
...
def classify_gemini_logs(logs: Sequence[Any] | None) -> list[dict[str, str]]:
    ...
    if (
        "prepayment credits are depleted" in lower
        or "billing" in lower
        or "결제/크레딧 고갈" in lower
    ):
        add(
            "billing_depleted",
            "critical",
...
def format_ops_markdown(
    *,
    ...
    if gemini_comparison.get("has_baseline"):
        ...
        lines.append("- Baseline 없음 — 같은 모드 Run이 1회 이상 쌓이면 절감률을 비교합니다.")
